...
The report keeps resource layout, helper resolution, runtime startup, and boundary status separate. Any probe-eligible executable other than the active PATH CLI requires explicit selection and remains alternative evidence even when it has the same Codex version. Among alternative candidates found under the known `%LOCALAPPDATA%` installation roots or `%CODEX_HOME%\packages\standalone`, a same-version candidate is preferred over a newer candidate. If no same-version alternative exists, the Canary may offer a newer one after checking that executable's own sandbox command; the version mismatch is reported as additional information and never determines the result target. Every alternative result applies only to the selected executable.
