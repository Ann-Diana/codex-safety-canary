...
export function selectCodexProbePlan(inventory) {
  ...
  for (const matchingCandidate of inventory.matchingCompleteBundles || []) {
    if (matchingCandidate.probeEligible === false
        || matchingCandidate.sandboxState !== WINDOWS_SANDBOX_FEATURE_STATES.AVAILABLE
        || !sandboxCommandContractSupported(matchingCandidate.sandboxCommandContract)) continue;
    addCandidate({
      ...
      sandboxCommandContract: matchingCandidate.sandboxCommandContract,
      testedBundleMetadata: buildProbeBundleMetadata(matchingCandidate),
      standaloneResourceBinding: matchingCandidate.standaloneResourceBinding || null,
  ...
  for (const newerCandidate of inventory.newerCompleteBundles || []) {
    if (newerCandidate.probeEligible === false
        || newerCandidate.sandboxState !== WINDOWS_SANDBOX_FEATURE_STATES.AVAILABLE
        || !sandboxCommandContractSupported(newerCandidate.sandboxCommandContract)) continue;
    addCandidate({
      ...
      sandboxCommandContract: newerCandidate.sandboxCommandContract,
      testedBundleMetadata: buildProbeBundleMetadata(newerCandidate),
      standaloneResourceBinding: newerCandidate.standaloneResourceBinding || null,
      scopeNote: describeAlternativeExecutableScope({
        versionMismatch: true,
        testedVersion: newerCandidate.version,
        activeVersion: inventory.codexVersion || null,
...
export function getCodexInventory(env = process.env) {
  ...
  for (const candidatePath of bundlePaths) {
    ...
    completeBundles.push({
      ...
      sandboxCommandContract: candidateSandboxFeature.commandContract,
    });
  }
...
export function buildRuntimeDiagnosticsForTarget(inventory = {}, sandbox = null, target = DIAGNOSTIC_TARGETS.ACTIVE_CLI, options = {}) {
  ...
  if (sandbox?.setupFailureCode === 'EXECUTABLE_IDENTITY_MISMATCH') {
    add(
      ...
      RECOMMENDATION_SEVERITIES.PROTECTION_NOT_CONFIRMED,
      'Standalone executable identity changed',
      ...
      'Refresh the inventory and select a probe-eligible executable whose package identity can be confirmed.'
    );
  }
