...
export const SHARE_SAFE_SHARING_NOTICE = Object.freeze({
  riskNotice: 'Share-safe output reduces disclosure risk but guarantees neither anonymity nor secrecy.',
  reviewNotice: 'Review it before public sharing.',
...
export function formatShareSafeSharingNotice(notice = SHARE_SAFE_SHARING_NOTICE) {
  const riskNotice = notice.riskNotice || SHARE_SAFE_SHARING_NOTICE.riskNotice;
  const redacted = formatEnglishList(notice.redactedCategories || []);
  const retained = formatEnglishList(notice.retainedCategories || []);
  const review = notice.reviewBeforePublicSharing ? ` ${notice.reviewNotice || SHARE_SAFE_SHARING_NOTICE.reviewNotice}` : '';
  return `${riskNotice}${review} It is designed to remove ${redacted}. It retains ${retained}.`;
}
...
const EXPECTED_BOUNDARY_LOCATIONS = Object.freeze(['inside', 'outside']);
const EXPECTED_BOUNDARY_PROBE_COUNT = EXPECTED_BOUNDARY_METHODS.length * EXPECTED_BOUNDARY_LOCATIONS.length;

export const PROCESS_START_EVIDENCE_STATES = Object.freeze({
  CONFIRMED: 'CONFIRMED',
  NOT_CONFIRMED: 'NOT CONFIRMED',
  NOT_REPORTED: 'NOT REPORTED',
});

export function processStartEvidenceStatus(value) {
  if (value === true) return PROCESS_START_EVIDENCE_STATES.CONFIRMED;
  if (value === false) return PROCESS_START_EVIDENCE_STATES.NOT_CONFIRMED;
  return PROCESS_START_EVIDENCE_STATES.NOT_REPORTED;
}

function projectProcessStartMarker(value) {
  if (value === true || value === false) return value;
  return null;
}

function normalizeBoundaryMethod(method) {
...
function expectedBoundaryProbeId(method, location) {
  return `${location}-workspace-${method}`;
}

function expectedBoundaryMethod(definition) {
  const explicitMethod = normalizeBoundaryMethod(definition?.method);
  if (explicitMethod) return explicitMethod;
  for (const method of EXPECTED_BOUNDARY_METHODS) {
    if (EXPECTED_BOUNDARY_LOCATIONS.some((location) => definition?.id === expectedBoundaryProbeId(method, location))) return method;
    if (definition?.id === `host-calibration-${method}`) return method;
  }
  return null;
}

function boundaryProbeProcessMarkerIsConfirmed(sandbox, probe) {
  const method = normalizeBoundaryMethod(probe?.method);
  const location = probe?.location;
  if (!method || !EXPECTED_BOUNDARY_LOCATIONS.includes(location)) return false;
  const expectedId = expectedBoundaryProbeId(method, location);
  return probe.id === expectedId
    && probe.targetId === expectedId
    && normalizeBoundaryMethod(probe.reportedRuntime) === method
    && probe.targetIdentityMatched === true
    && probe.runMarker === sandbox?.marker
    && probe.codexExecutableIdentity === sandbox?.codexExecutableIdentity
    && probe.codexProcessStarted === true;
}

export function boundaryProbeProcessMarkersAreComplete(sandbox) {
  const probes = sandbox?.probes;
  const runMarker = sandbox?.marker;
  const executableIdentity = sandbox?.codexExecutableIdentity;
  if (!Array.isArray(probes) || probes.length !== EXPECTED_BOUNDARY_PROBE_COUNT) return false;
  if (typeof runMarker !== 'string' || !runMarker || typeof executableIdentity !== 'string' || !executableIdentity) return false;

  const expectedIds = new Set(EXPECTED_BOUNDARY_METHODS.flatMap((method) => (
    EXPECTED_BOUNDARY_LOCATIONS.map((location) => expectedBoundaryProbeId(method, location))
  )));
  ...
  for (const probe of probes) {
    ...
    if (!method || !EXPECTED_BOUNDARY_LOCATIONS.includes(location)) return false;
    const expectedId = expectedBoundaryProbeId(method, location);
    if (!expectedIds.has(expectedId) || seenIds.has(expectedId)) return false;
    if (!boundaryProbeProcessMarkerIsConfirmed(sandbox, probe)) return false;
    seenIds.add(expectedId);
  }
  return seenIds.size === expectedIds.size;
}

export function boundaryProbeProcessStartEvidence(sandbox) {
  const probes = Array.isArray(sandbox?.probes) ? sandbox.probes : [];
  const expectedIds = EXPECTED_BOUNDARY_METHODS.flatMap((method) => (
    EXPECTED_BOUNDARY_LOCATIONS.map((location) => expectedBoundaryProbeId(method, location))
  ));
  const confirmedCount = expectedIds.filter((expectedId) => {
    const matches = probes.filter((probe) => probe?.id === expectedId);
    return matches.length === 1 && matches[0].codexProcessStarted === true;
  }).length;
  return {
    expectedCount: EXPECTED_BOUNDARY_PROBE_COUNT,
    confirmedCount,
    aggregate: processStartEvidenceStatus(sandbox?.codexProcessStarted),
  };
}

export function formatBoundaryProbeProcessStartEvidence(sandbox) {
  const evidence = boundaryProbeProcessStartEvidence(sandbox);
  return `Boundary probe process-start evidence: ${evidence.confirmedCount}/${evidence.expectedCount} CONFIRMED; aggregate: ${evidence.aggregate}`;
}

export function smokeTestStatus(smoke) {
  if (!smoke) return 'NOT RUN';
  if (smoke.passed === true) return 'PASS';
  if (smoke.passed === false) return 'FAIL';
  return 'NOT REPORTED';
}

export function formatSmokeProcessStartEvidence(smoke) {
  return `Runtime smoke test: ${smokeTestStatus(smoke)}; process-start evidence: ${processStartEvidenceStatus(smoke?.codexProcessStarted)}`;
}

function addProcessStartEvidenceProjection(sandbox) {
  if (!sandbox) return null;
  return {
    ...sandbox,
    processStartEvidence: processStartEvidenceStatus(sandbox.codexProcessStarted),
    boundaryProbeProcessStartEvidence: boundaryProbeProcessStartEvidence(sandbox),
    smokeStatus: smokeTestStatus(sandbox.smoke),
    smokeProcessStartEvidence: processStartEvidenceStatus(sandbox.smoke?.codexProcessStarted),
    smoke: sandbox.smoke ? {
      ...sandbox.smoke,
      processStartEvidence: processStartEvidenceStatus(sandbox.smoke.codexProcessStarted),
    } : sandbox.smoke,
    probes: (sandbox.probes || []).map((probe) => ({
      ...probe,
      processStartEvidence: processStartEvidenceStatus(probe.codexProcessStarted),
    })),
  };
}
...
function sandboxProcessStartIsConfirmed(sandbox) {
  return sandbox?.codexProcessStarted === true
    && boundaryProbeProcessMarkersAreComplete(sandbox)
    && sandbox?.smoke?.codexProcessStarted === true;
}
...
function createExecutableRunBinding(executablePath) {
  const identity = resolveFilesystemIdentity(executablePath);
  if (!executablePath || identity?.proven !== true || !identity.objectIdentity?.key || identity.key !== identity.objectIdentity.key) return null;
  return {
...
export function selectCodexProbePlan(inventory) {
  ...
  if (activeProbeEligible
    ...
    addCandidate({
      ...
      executableRunBinding: createExecutableRunBinding(inventory.activeCodexPath),
...
export function runSandboxCommand(command, cwd, options = {}) {
  if (options.executableRunBinding) {
    const binding = validateSelectedExecutableBinding(options.executableRunBinding, options.codexExe);
    if (!binding.valid) {
      return {
        status: null,
        stdout: '',
  ...
  const result = (options.invokeCodex || invokeCodex)(args, { cwd, timeout: 45_000, codexExe: options.codexExe });
...
export function deriveProbeExecutionEvidence({ before, after, result, definition, hostCalibration = null }) {
  ...
  const reportedRuntime = normalizeBoundaryMethod(record?.runtime);
  const runtimeIdentityMatched = reportedRuntime === expectedBoundaryMethod(definition);
  const reportedTargetPathMatched = reportedRuntime === 'cmd'
    ? (record?.targetPath == null
      || (typeof record.targetPath === 'string' && record.targetPath.length > 0 && sameWindowsPath(record.targetPath, definition.file)))
    : (typeof record?.targetPath === 'string' && record.targetPath.length > 0 && sameWindowsPath(record.targetPath, definition.file));
  const targetIdentityMatched = Boolean(record) && runtimeIdentityMatched && record.targetId === definition.id
    && reportedTargetPathMatched;
  ...
  return {
    commandStarted,
    operationAttempted,
    reportedRuntime: record?.runtime || null,
    runtimeIdentityMatched,
    targetId: record?.targetId ?? null,
    targetPath: definition.file,
    reportedTargetPath: record?.targetPath ?? null,
    exitCode,
  ...
  if (sandbox?.setupFailureCode === 'EXECUTABLE_IDENTITY_MISMATCH') {
    ...
    add(
      ...
      standaloneBindingFailed
        ? 'The affected sandbox invocation was not started because the selected executable could not be confirmed as the canonical executable bound to the standalone package resources.'
        : 'The affected sandbox invocation was not started because the selected executable no longer matched the exact filesystem object that was explicitly selected.',
...
export function createBoundaryProbeRecord({ definition, method, expected, after, result, classification, runMarker, codexExecutableIdentity }) {
  return {
    id: definition.id, label: definition.label, method, location: definition.location, expected,
    observed: after ? 'RETAINED' : 'DELETED', assessment: classification.assessment,
    codexProcessStarted: result.codexProcessStarted,
    runMarker,
    codexExecutableIdentity,
    reportedRuntime: classification.reportedRuntime,
    ...
    reportedTargetPath: classification.reportedTargetPath,
    targetIdentityMatched: classification.targetIdentityMatched,
    hostCalibrationStatus: classification.hostCalibrationPassed ? 'PASS' : 'FAIL',
...
export function finalizeSandboxProbeRun(result) {
  const finalized = { ...result, codexProcessStarted: false };
  finalized.codexProcessStarted = finalized.status === 'COMPLETED'
    && boundaryProbeProcessMarkersAreComplete(finalized);
  return finalized;
}
...
export function runSandboxProbes(options = {}) {
  ...
  const executableRunBinding = alternativeTarget
    ? options.selectedExecutableBinding
    : options.executableRunBinding;
  if (alternativeTarget) {
    ...
    if (!selectedBinding.valid) {
      return {
        ...
        error: 'The affected sandbox invocation was not started because the selected alternative executable no longer matched its explicit selection binding.',
      };
    }
  ...
  if (standaloneResourcesUsed) {
    ...
    if (!resourceBinding.valid) {
      return {
        ...
        error: 'The affected sandbox invocation was not started because the selected executable no longer matched the canonical standalone package executable.',
      };
    }
  ...
  const runBinding = validateSelectedExecutableBinding(executableRunBinding, options.codexExe);
  if (!runBinding.valid) {
    return {
      status: 'SETUP_FAILED', probes: [], cleanup: null, ...targetMetadata,
      executableBindingStatus: 'FAILED', setupFailureCode: runBinding.code,
      error: 'The affected sandbox invocation was not started because the executable no longer matched the identity bound for that invocation.',
    };
  }
  targetMetadata.executableBindingStatus = 'CONFIRMED';
  ...
  if (!smokePassed) {
    return { status: 'SETUP_FAILED', probes, layout, marker, smoke, hostPreflight, hostCalibrations,
      ...targetMetadata, codexProcessStarted: false,
  ...
  try {
    for (const method of methods) {
      ...
      for (const location of ['inside', 'outside']) {
        ...
        probes.push(createBoundaryProbeRecord({
          definition, method: method.method, expected, after, result, classification,
          runMarker: marker, codexExecutableIdentity: targetMetadata.codexExecutableIdentity,
        }));
      }
    }
    return finalizeSandboxProbeRun({ status: 'COMPLETED', probes, layout, marker, hostPreflight, hostCalibrations, smoke,
      ...targetMetadata, permissionProfile, error: null });
  } catch (error) {
    return finalizeSandboxProbeRun({ status: 'ERROR', probes, layout, marker, hostPreflight, hostCalibrations, smoke,
...
export function summarizeAssessment(inventory, rules, sandbox, options = {}) {
  ...
  const normalizedProbes = probes.map((probe) => ({ ...probe, normalizedMethod: normalizeBoundaryMethod(probe.method) }));
  const boundaryGapMethods = [...new Set(normalizedProbes
    .filter((probe) => probe.location === 'outside'
      && probe.assessment === 'CRITICAL_GAP'
      && boundaryProbeProcessMarkerIsConfirmed(sandbox, probe))
  ...
  const hostCalibrations = new Map((sandbox?.hostCalibrations || []).map((item) => [normalizeBoundaryMethod(item.method), item]));
  const methodResults = EXPECTED_BOUNDARY_METHODS.map((method) => {
    ...
    const gap = exactPair
      && inside.assessment === 'EXPECTED'
      && inside.observed === 'DELETED'
      && outside.assessment === 'CRITICAL_GAP'
      && boundaryProbeProcessMarkerIsConfirmed(sandbox, outside);
    return {
      method,
      pass,
      gap,
...
export function buildSupportPayload(report, env = process.env) {
  ...
  const supportPayload = {
    ...
    tool: report.tool,
    sharingNotice: {
      riskNotice: SHARE_SAFE_SHARING_NOTICE.riskNotice,
      reviewNotice: SHARE_SAFE_SHARING_NOTICE.reviewNotice,
    ...
    sandbox: report.sandbox ? {
      ...
      codexProcessStarted: projectProcessStartMarker(report.sandbox.codexProcessStarted),
      processStartEvidence: processStartEvidenceStatus(report.sandbox.codexProcessStarted),
      boundaryProbeProcessStartEvidence: boundaryProbeProcessStartEvidence(report.sandbox),
      smokeStatus: smokeTestStatus(report.sandbox.smoke),
      smokeProcessStartEvidence: processStartEvidenceStatus(report.sandbox.smoke?.codexProcessStarted),
      ...
      smoke: report.sandbox.smoke ? {
        ...
        codexProcessStarted: projectProcessStartMarker(report.sandbox.smoke.codexProcessStarted),
        processStartEvidence: processStartEvidenceStatus(report.sandbox.smoke.codexProcessStarted),
      ...
      probes: (report.sandbox.probes || []).map((probe) => ({
        ...
        codexProcessStarted: projectProcessStartMarker(probe.codexProcessStarted),
        processStartEvidence: processStartEvidenceStatus(probe.codexProcessStarted),
        ...
        targetIdentityMatched: probe.targetIdentityMatched === true,
...
export function renderSupportReport(report) {
  ...
  if (!report.sandbox) add('(not run)');
    ...
    add(formatBoundaryProbeProcessStartEvidence(report.sandbox));
    add(`Sandbox syntax used: ${report.sandbox.commandSyntax || 'UNKNOWN'}`);
    add(formatSmokeProcessStartEvidence(report.sandbox.smoke));
    ...
    for (const probe of report.sandbox.probes || []) {
      ...
      add(`  Process-start evidence: ${processStartEvidenceStatus(probe.codexProcessStarted)}`);
      ...
      add(`  Target ID: ${probe.targetId ?? '(not reported)'}; target identity matched: ${probe.targetIdentityMatched ? 'yes' : 'no'}`);
...
export function writeReport({ inventory, rules = [], sandbox = null, appRoot = getAppRoot(), assessmentMode = ASSESSMENT_MODES.GUIDED, boundaryAssessmentDeclined = false }) {
  ...
  const reportSandbox = addProcessStartEvidenceProjection(sandbox);
  const payload = {
    schemaVersion: 1,
...
export function renderTextReport(report) {
  ...
  if (!report.sandbox) {
    ...
    for (const probe of report.sandbox.probes || []) {
      ...
      add(`  Target ID: ${probe.targetId ?? '(not reported)'}`);
      add(`  Target path: ${probe.targetPath || '(not recorded)'}`);
      add(`  Child-reported target path: ${probe.reportedTargetPath ?? '(not reported)'}`);
