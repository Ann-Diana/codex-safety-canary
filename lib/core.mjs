...
function realPathKey(filePath) {
  try {
    return pathKey(fs.realpathSync.native(filePath));
  } catch {
...
function inspectStandaloneDirectory(releaseDir, source = 'release', aliasPath = null) {
  let resolved = path.resolve(releaseDir);
  ...
  const aliases = aliasPath && path.resolve(aliasPath).toLowerCase() !== resolved.toLowerCase() ? [path.resolve(aliasPath)] : [];
...
export function discoverStandalonePackages(codexHome = getCodexHome()) {
  ...
  for (const candidate of candidates) {
    const key = realPathKey(candidate.dir);
