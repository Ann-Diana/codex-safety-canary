...
export function resolveFilesystemIdentity(filePath) {
  ...
  let resolutionMethod = 'LEXICAL_FALLBACK';
  for (const candidate of linkTarget ? [linkTarget, absolutePath] : [absolutePath]) {
    try {
      ...
      if (usableRealPath(nativeResult)) {
        ...
        resolutionMethod = 'REALPATH_NATIVE';
        break;
      }
    } catch {}
    try {
      ...
      if (usableRealPath(fallbackResult)) {
        ...
        resolutionMethod = 'REALPATH_FALLBACK';
        break;
      }
    } catch {}
  }
  ...
  return {
    ...
    resolutionMethod,
    key: process.platform === 'win32' ? normalizedPath.toLowerCase() : normalizedPath,
  };
}
...
export function discoverStandalonePackages(codexHome = getCodexHome()) {
  ...
  const packagesByRealPath = new Map();
  for (const candidate of candidates) {
    const identity = resolveFilesystemIdentity(candidate.dir);
    const key = identity?.key || pathKey(candidate.dir);
    const inspected = inspectStandaloneDirectory(candidate.dir, candidate.source, candidate.dir, identity);
    if (!filePresent(inspected.executablePath) && !inspected.resourcesFound && !inspected.codexPathFound) continue;
    const existing = packagesByRealPath.get(key);
    if (existing) {
      if (!existing.sources.includes(candidate.source)) existing.sources.push(candidate.source);
      existing.source = existing.sources.includes('current') ? 'current' : existing.sources[0];
      for (const alias of inspected.aliases || []) {
        if (!existing.aliases.some((value) => pathKey(value) === pathKey(alias))) existing.aliases.push(alias);
      }
      continue;
    }
    packagesByRealPath.set(key, inspected);
  }
  const packages = [...packagesByRealPath.values()];
...
function findStandalonePackageForExecutable(executablePath, packages = []) {
  ...
  const executableKey = realPathKey(executablePath);
  return packages.find((bundle) => {
    const bundleExecutableKey = realPathKey(bundle.executablePath);
    const bundleReleaseKey = realPathKey(bundle.releaseDir);
    return bundleExecutableKey === executableKey || executableKey.startsWith(`${bundleReleaseKey}${path.sep}`);
  }) || null;
}
