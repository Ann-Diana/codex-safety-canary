...
export function createCanaryCli(dependencies) {
  ...
  function printInventory(inventory, assessmentMode = ASSESSMENT_MODES.GUIDED, sandboxRuntime = 'NOT TESTED') {
    ...
    writeLine(`CLI bundle:    ${inventory.activeBundle?.complete ? 'complete' : 'incomplete'}`);
    if (inventory.activeBundle?.missing?.length) writeLine(`Missing files: ${inventory.activeBundle.missing.join(', ')}`);
    if (inventory.matchingCompleteBundles?.length) writeLine(`Matching complete bundle: ${inventory.matchingCompleteBundles[0].executablePath}`);
    if (inventory.newerCompleteBundles?.length) {
      writeLine(`Newer complete bundle:    ${inventory.newerCompleteBundles[0].executablePath}`);
    ...
    writeLine(`Sandbox runtime:        ${sandboxRuntime}`);
  ...
  function printConsolidatedResult(summary) {
    ...
    writeLine(`  Bundle:   ${summary.activeCli?.bundleStatus || 'UNKNOWN'}`);
  ...
  async function chooseSandboxProbePlan(inventory) {
    ...
    if (plan.source === 'NEWER_COMPLETE_BUNDLE') {
      writeLine('A newer complete local Codex bundle was found:');
      ...
      writeLine('A complete local bundle with the same Codex version was found:');
      writeLine(`  ${plan.codexExe}`);
    }
    ...
    const prompt = plan.source === 'NEWER_COMPLETE_BUNDLE'
      ? 'Test the newer complete bundle separately? [Y/N]: '
      : 'Use the matching complete bundle for live probes? [Y/N]: ';
  ...
  async function runGuided() {
      ...
      if (probePlan?.declined) {
        ...
        if (YES_ANSWERS.has(answer)) {
          sandbox = runSandboxProbes({
            ...
            sandboxWindowsState: inventory.sandboxWindowsState,
    ...
    if (sandbox?.layout?.runDir) {
      try {
        ...
        sandbox.cleanup = 'Disposable run folder removed before final reports were written.';
      } catch (error) {
        sandbox.cleanup = `Cleanup warning: ${error.message}`;
